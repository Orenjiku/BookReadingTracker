...
const ReaderBookView = ({ readerBookList, isEditing }: { readerBookList: ReaderBookITF[]; isEditing: boolean }) => {
  return (
    <div className='row-start-10 row-end-19 col-start-2 col-end-3 font-SortsMillGoudy-400'>
      {readerBookList.map(readerBook => (
        <ReaderBook key={readerBook.rb_id} readerBook={readerBook} isEditing={isEditing} />
      ))}
    </div>
  )
}
